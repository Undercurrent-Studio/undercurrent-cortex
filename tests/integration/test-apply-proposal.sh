...
run_apply_proposal() {
  ...
  export PROPOSALS_FILE="$_TEST_TMPDIR/.claude/undercurrent-proposals.local.md"
...
create_proposals_file "$_TEST_TMPDIR/.claude/undercurrent-proposals.local.md" \
...
assert_file_contains "reject_status_changed" "$_TEST_TMPDIR/.claude/undercurrent-proposals.local.md" "status=rejected"

# Test 3: Empty proposals file (no pending proposals)
setup_test
> "$_TEST_TMPDIR/.claude/undercurrent-proposals.local.md"
result=$(run_apply_proposal "approve")
...
assert_file_contains "approve_sets_date" "$_TEST_TMPDIR/.claude/undercurrent-proposals.local.md" "applied_date="
