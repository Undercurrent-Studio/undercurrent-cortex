...
**Failure example**: Signal Intelligence Phase 0 assumed `TickerData` had 6 field names without reading the type definition — all 6 were wrong.
...
**Failure example**: `convergence_events` query used `event_date` column — correct column is `detected_at`. The V_SIMPLIFY audit found the Trending Stocks widget was completely dead from this.
...
**Failure example**: Multiple pipeline modules independently implemented batch chunking logic. `batchUpsert()` in `src/lib/utils/batch-upsert.ts` already existed.

### 4. File paths verified
Are all file paths verified to exist (or explicitly marked as `[NEW]`)? No phantom paths.

**Failure example**: Plan referenced `src/lib/scoring/v8-engine.ts` after V8 was already removed in favor of V10/V11.
...
**Failure example**: USPTO patent search (200 companies x 2 calls x 1.5s = 500s) was planned for the Vercel pipeline. Exceeded 300s limit. Had to be moved to weekly GitHub Actions workflow.
...
**Failure example**: Price history backfill with PIPELINE_BATCH_SIZE=600 produced `.in()` queries returning 219K rows, silently truncated at 50K.
...
**Failure example**: `/database` route was missing from middleware matcher — session tokens not refreshed, expired JWTs caused empty pages via RLS.
...
**Failure example**: `getServerEnv()` validates ALL 10 server env vars eagerly. Missing `EIA_API_KEY` crashed the checkout route.
...
**Failure example**: Migration 055 (ticker_aliases) referenced tickers via FK that didn't exist in stocks table (TWTR→X rename).
